...
def train_voxel_grid(viewpoint_camera, pc, pipe, bg_color: torch.Tensor, scaling_modifier=1.0,
                     override_color=None, basicfunction=None, GRsetting=None, GRzer=None):
    """
    Renderer for the fixed-grid Gaussian model.
    Positions remain static; only color/opacity/scale/rotation/time envelope are learned.
    """
    startime = time.time()
    ...
    world_view = viewpoint_camera.world_view_transform.to(device)
    proj_matrix = viewpoint_camera.full_proj_transform.to(device)
    ...
    if hasattr(GRsetting, "_fields") and "debug" in GRsetting._fields:
        settings_kwargs["debug"] = getattr(pipe, "debug", False)

    raster_settings = GRsetting(**settings_kwargs)
    rasterizer = GRzer(raster_settings=raster_settings)

    means3D = pc.get_xyz
    means2D = screenspace_points
    base_opacity = pc.get_opacity.reshape(-1, 1)

    timestamp = viewpoint_camera.timestamp
    if not torch.is_tensor(timestamp):
        timestamp = torch.tensor(float(timestamp), device=base_opacity.device)
    else:
        timestamp = timestamp.to(device=base_opacity.device, dtype=base_opacity.dtype)
    trbfcenter = pc.get_trbfcenter.reshape(-1, 1)
    trbfscale = pc.get_trbfscale.reshape(-1, 1)

    pointtimes = torch.full_like(trbfcenter, float(timestamp))

    trbfdistanceoffset = pointtimes - trbfcenter
    trbfdistance = trbfdistanceoffset / torch.exp(trbfscale)
    trbfoutput = basicfunction(trbfdistance)
    opacity = base_opacity * trbfoutput
    pc.trbfoutput = trbfoutput
